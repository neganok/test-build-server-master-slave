...
const thongBaoMatKetNoi = (url) => { const slave = danhSachSlave.find(s => s.url === url); if (slave) bot.sendMessage(ID_NHOM, `⚠️ *Slave ${slave.tenMay} mất kết nối!*`, {parse_mode: 'Markdown'}); danhSachSlave = danhSachSlave.filter(s => s.url !== url); };
...
app.post(`/bot${TOKEN}`, (req, res) => {
  ...
  if (noiDung === '/status') {
    Promise.all([new Promise(resolve => chayNeofetch(ketQua => resolve({ loai: 'master', ten: TEN_MAY, thoigian: layThoiGianHoatDong(ketQua) }))), ...danhSachSlave.map(s => Promise.resolve({ loai: 'slave', ten: s.tenMay, thoigian: s.thoigianHoatDong }))]).then(tatCa => { let ketQua = `🟢 *Bots online (${tatCa.length}):*\n`; tatCa.forEach(b => ketQua += `${b.loai === 'master' ? '👑 *Master*' : '🤖 *Slave*'}: ${b.ten} (Hoạt động: ${b.thoigian})\n`); bot.sendMessage(tinNhan.chat.id, ketQua, {parse_mode: 'Markdown'}); res.sendStatus(200); });
    return;
  }
  if (noiDung.startsWith('/cmd')) {
    ...
    if (LA_MASTER) {
      ...
      danhSachSlave.forEach(({ url, tenMay }) => {
        const guiLenh = https.request({ hostname: new URL(url).hostname, path: '/exec', method: 'POST', headers: { 'Content-Type': 'application/json' } }, phanHoi => { let duLieu = ''; phanHoi.on('data', chunk => duLieu += chunk); phanHoi.on('end', () => bot.sendMessage(tinNhan.chat.id, `💻 *Slave ${tenMay}:*\n\`\`\`\n${duLieu.trim()}\n\`\`\``, {parse_mode: 'Markdown'})); });
        guiLenh.on('error', loi => bot.sendMessage(tinNhan.chat.id, `❌ *Slave ${tenMay} lỗi:* ${loi.message}`, {parse_mode: 'Markdown'})); guiLenh.write(JSON.stringify({ cmd: lenh })); guiLenh.end();
      });
...
app.post('/register', (req, res) => { const { port, url, hostname, uptime, report } = req.body; if (!port || !url || !hostname) return res.sendStatus(400); const bayGio = Date.now(); const slaveTonTai = danhSachSlave.findIndex(s => s.url === url); if (slaveTonTai >= 0) danhSachSlave[slaveTonTai] = { port, url, tenMay: hostname, thoigianHoatDong: uptime, lanCuoiPing: bayGio }; else danhSachSlave.push({ port, url, tenMay: hostname, thoigianHoatDong: uptime, lanCuoiPing: bayGio }); bot.sendMessage(ID_NHOM, `📩 *Slave đăng ký:*\n*Tên máy:* ${hostname}\n*Thời gian:* ${uptime}\n*URL:* ${url}\n\n\`\`\`\n${report || ''}\n\`\`\``, {parse_mode: 'Markdown'}); res.sendStatus(200); });
