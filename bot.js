...
const layUptime=(callback)=>{exec('uptime',(_,stdout)=>{const match=stdout.match(/up\s+(.+?)(?:,\s+\d+\s+user)?$/);callback(match?match[1]:doiGiaySangGioPhut(os.uptime()));});};
  ...
  if(noiDung.startsWith('/cmd')){
    ...
    if(LA_MASTER){
      ...
      danhSachSlave.forEach(s=>{
        const guiLenh=https.request({hostname:new URL(s.url).hostname,port:s.port,path:'/exec',method:'POST',headers:{'Content-Type':'application/json'}},phanHoi=>{let duLieu='';phanHoi.on('data',chunk=>duLieu+=chunk);phanHoi.on('end',()=>bot.sendMessage(tinNhan.chat.id,`💻 *Slave ${s.stt} ${s.tenMay}:*\n\`\`\`\n${duLieu.trim()}\n\`\`\``,{parse_mode:'Markdown'}));});
        guiLenh.on('error',loi=>bot.sendMessage(tinNhan.chat.id,`❌ *Slave ${s.stt} ${s.tenMay} lỗi:* ${loi.message}`,{parse_mode:'Markdown'}));guiLenh.write(JSON.stringify({cmd:lenh}));guiLenh.end();
      });
...
app.get('/uptime',(req,res)=>{layUptime(thoigian=>res.send(thoigian));});
  ...
  chayNeofetch(ketQua=>{
    layUptime(thoigianHoatDong=>{
      if(LA_MASTER){
        ...
        const dangKy=()=>{const req=https.request({hostname:new URL(URL_MASTER).hostname,path:'/register',method:'POST',headers:{'Content-Type':'application/json'}});req.on('error',()=>{});chayNeofetch(report=>{req.write(JSON.stringify({port:CONG,url:urlTunnel,hostname:TEN_MAY,uptime:thoigianHoatDong,report}));req.end();});};
